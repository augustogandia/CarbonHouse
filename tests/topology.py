...
def get_assembly_topology(meshes, panel_width):
    topology = {}
    ...
    # To do: 
    # 1) turn this into the assembly class
    # 2) add attributes assigned by the user in Rhino
    
    # Add a network node (panel) and its attributes
    for key, mesh in meshes.items():         
        ...
        topology_network.node_attribute(key, "panel_width", panel_width)

    # Add a network edge (connection) 
    ...
    #Iter network nodes (panels), evaluate their vertices and set their type
    for node_key in topology_network.nodes():
        node_mesh = topology_network.node_attribute(node_key, "mesh")
        nbrs_key = topology_network.neighbors(node_key)
        
        #Check repeated neighbor vertices 
        mesh_vertices_coordinates = get_mesh_vertices_coordinates(node_mesh)
        
        vertex=0
        for vertex_coordinates in mesh_vertices_coordinates:
            #Iter neighbors
            vertex_type=0
            extrusion_normals=[topology_network.node_attribute(node_key, "mesh_normal")]
            for nbr_key in nbrs_key:
                #Get mesh
            ...
            node_mesh.vertex_attribute(vertex, "vertex_type", vertex_type)
            node_mesh.vertex_attribute(vertex, "neighbor_normals", extrusion_normals)

            vertex+=1

    #Calculate extrusion vector and store it as joint attribute (edge)
...
if __name__ == "__main__":  
    ...
    #get topology
    topology, topology_network = get_assembly_topology (meshes, panel_width)
